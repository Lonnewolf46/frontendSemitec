...
export default function CreatLesson() {
  ...
  const createLesson = async () => {
    try {
      console.log(JSON.stringify({ name : name, level_id : level_id, words : words, description : description, min_time : min_time, min_mistakes : min_mistakes }));
      const response = await fetch(
        "http://25.37.76.172:5000/teacher/lessons/create",
        {
          ...
          body: JSON.stringify({ name : name, level_id : level_id, words : words, description : description, min_time : min_time, min_mistakes : min_mistakes }), 
        }
      );
